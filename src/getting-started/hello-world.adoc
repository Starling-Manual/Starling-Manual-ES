...
Tipicamente, esto significa que tiene que aplicar el siguiente cambio:
...
En ese caso, vas a tener que editar el "AIR application descriptor", que probablemente se llame `HelloWorld-app.xml` o parecido.
Busca el nodo XML `renderMode` (que podría estar comentado) y cambia el valor por `direct`.
...
NOTE: Lo que hemos hecho aquí is darle permiso al runtime para acceder al GPU.
